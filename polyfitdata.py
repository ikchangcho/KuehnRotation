...
for file_name in os.listdir(directory):
    if file_name.startswith('240624_waterspike_') and file_name.endswith('.csv'):
...
for i, (color, label, linestyle, linewidth) in enumerate(zip(colors, labels, linestyles, linewidths)):
    ...
    coefficients = np.polyfit(hours, co2, 2)
    polynomial = np.poly1d(coefficients)
    polyfit = polynomial(hours)

    ax.plot(hours, co2, color=color, linestyle=linestyle, linewidth=linewidth, label=label)
    ax.plot(hours, polyfit, color=color, linestyle=linestyle, linewidth=linewidth, label=label)
